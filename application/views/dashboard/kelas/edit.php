			...
			<div class="row">
				<div class="col-12">
                  ...
                  <?php foreach($kelas as $k){?>
							<form method="post" action="<?php echo base_url('inipanel/kelas_edit_aksi')?>">
								<input type="hidden" name="<?=$this->security->get_csrf_token_name();?>"
									value="<?=$this->security->get_csrf_hash();?>" style="display: none">
                        <input type="hidden" name="id" value="<?=encrypt_url($k->kelas_id)."";?>">
								<div class="form-group row">
									<label class="col-sm-3 col-form-label">Kelas</label>
									<div class="col-sm-9">
										<input type="text" class="form-control" name="kelas" value="<?=$k->kelas_nama;?>"
											required>
									</div>
